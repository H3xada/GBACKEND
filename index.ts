...
app.get('/', (_req: Request, res: Response) => {
  res.send('Firefly Login Server Running.');
});

/**
 * Dashboard — serves the login/register HTML page to the Growtopia client.
 */
...
/**
 * Validate — middleman only.
 * Takes growId + password from the form, packs them into the token
 * format FireflyCore expects, and returns it. FireflyCore handles
 * all actual validation (wrong password, banned, new account, etc).
 */
app.all('/player/growid/login/validate', async (req: Request, res: Response) => {
  try {
    ...
    const token = Buffer.from(`growId|${growId}\npassword|${password}`).toString('base64');

    console.log(`[LOGIN] Passing through: ${growId}`);
...
/**
 * Checktoken redirect — preserves request body via 307.
 */
...
/**
 * Checktoken — middleman only.
 * Decodes the saved ltoken, repacks growId + password in the correct
 * format, and returns it. FireflyCore handles the actual validation.
 */
app.all('/player/growid/validate/checktoken', async (req: Request, res: Response) => {
  try {
    ...
    if (!refreshToken || !clientData) {
      return res.status(200).json({ status: 'error', message: 'Missing refreshToken or clientData' });
    }

    // Decode the stored token to extract growId + password
    const decodedToken = Buffer.from(refreshToken, 'base64').toString('utf-8');
    const lines = decodedToken.split('\n');
    let growId   = '';
    let password = '';
    for (const line of lines) {
      const [key, val] = line.split('|');
      if (key === 'growId')   growId   = val || '';
      if (key === 'password') password = val || '';
    }

    // Repack in same format — FireflyCore validates
    const token = Buffer.from(`growId|${growId}\npassword|${password}`).toString('base64');

    console.log(`[CHECKTOKEN] Passing through: ${growId}`);
...
app.listen(PORT, () => {
  console.log(`[SERVER] Firefly Login running on http://localhost:${PORT}`);
});
