...
// @note trust proxy - Vercel sits in front
...
app.get('/', (_req: Request, res: Response) => {
  res.send('GTPSX Login Server');
});

/**
 * @note dashboard endpoint - serves login/register HTML page
 */
...
/**
 * @note validate endpoint - handles both login and registration
 * Login:    growId + password (no email)    -> reg=0
 * Register: growId + password + email       -> reg=1
 */
app.all('/player/growid/login/validate', async (req: Request, res: Response) => {
  try {
    ...
    const passwordVerify = formData.password_verify ?? '';
    const email = formData.email ?? '';

    // Registration - email present means register form was submitted
    if (email) {
      if (growId.length < 3 || growId.length > 18) {
        return res.send(JSON.stringify({ status: 'error', message: 'Name must be between 3 and 18 characters.' }));
      }
      if (password.length < 4 || password.length > 18) {
        return res.send(JSON.stringify({ status: 'error', message: 'Password must be between 4 and 18 characters.' }));
      }
      if (password !== passwordVerify) {
        return res.send(JSON.stringify({ status: 'error', message: 'Passwords do not match.' }));
      }
      if (!email.includes('@') || !email.includes('.')) {
        return res.send(JSON.stringify({ status: 'error', message: 'Invalid email address.' }));
      }
...
/**
 * @note checktoken - redirects to validate/checktoken
 */
...
/**
 * @note validate/checktoken - handles token refresh
 */
